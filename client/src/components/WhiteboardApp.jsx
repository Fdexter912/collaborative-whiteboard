...
export default function WhiteboardApp({ roomId, userId }) {
    ...
    (event) => {
      ...
      switch (event.type) {
        ...
        case "draw.deleteStroke":
          setStrokes((prev) =>
            prev.filter((s) => s.id !== event.data.strokeId),
          );
          break;
  ...
  useSocketEvent("sync.events", ({ events, currentSeq }) => {
    console.log(
      `📦 Received ${events.length} sync events, current seq: ${currentSeq}`,
    );
  ...
  /**
   * Listen for sync complete (bulk event application)
   * NEW for Commit 3
   */
  useSocketEvent("sync.complete", (events) => {
    console.log(`📦 Sync complete: applying ${events.length} events`);
    ...
    <div style={dynamicStyles.container}>
      ...
      <div style={styles.header}>
        ...
        <div style={dynamicStyles.roomInfo}>
          ...
          <span
            style={{
              ...styles.syncStatus,
              color: syncStats.syncStatus?.syncing
                ? "#f39c12"
                : syncStats.hasGaps
                  ? "#e74c3c"
                  : "#27ae60",
          ...
          >
            {syncStats.syncStatus?.syncing
              ? "🔄 Syncing..."
              : syncStats.hasGaps
                ? `⚠️ Gap (${syncStats.pendingCount} pending)`
      ...
      {showStats && (
        ...
        >
          ...
          <div>
            Points: {strokes.reduce((sum, s) => sum + s.points.length, 0)}
          </div>
          <div
            style={{
              marginTop: "8px",
              borderTop: `1px solid ${darkMode ? "#34495e" : "#ddd"}`,
              paddingTop: "8px",
            }}
          >
          ...
          {syncStats.syncStatus?.syncing && (
            <div style={{ color: "f39c12" }}>⏳ Syncing...</div>
          )}
          {syncStats.syncStatus?.failedAttempts > 0 && (
            <div style={{ color: "#e74c3c" }}>
              Failed attempts: {syncStats.syncStatus.failedAttempts}
            </div>
          )}
          {/* Add manual sync button */}
          <button
            onClick={() => syncManager.requestFullSync()}
            ...
            style={{
              ...
              border: "none",
              borderRadius: "4px",
              cursor: syncStats.syncStatus?.syncing ? "not-allowed" : "pointer",
              opacity: syncStats.syncStatus?.syncing ? 0.5 : 1,
            }}
          >
            Force Sync
          </button>
